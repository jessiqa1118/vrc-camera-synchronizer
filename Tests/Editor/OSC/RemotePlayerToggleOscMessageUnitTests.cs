...
{
    [TestFixture]
    public class RemotePlayerToggleOscMessageUnitTests
    {
        [TestCase(true, "T")]
        [TestCase(false, "F")]
        public void Ctor_WithToggle_BuildsOscMessage(bool toggle, string expectedTypeTag)
        {
            var message = new RemotePlayerToggleOscMessage(toggle);

            Assert.AreEqual(OSCCameraEndpoints.RemotePlayer, message.Address);
