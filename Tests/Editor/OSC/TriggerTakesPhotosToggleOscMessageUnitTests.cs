...
{
    [TestFixture]
    public class TriggerTakesPhotosToggleOscMessageUnitTests
    {
        [TestCase(true, "T")]
        [TestCase(false, "F")]
        public void Ctor_WithToggle_BuildsOscMessage(bool toggle, string expectedTypeTag)
        {
            var message = new TriggerTakesPhotosToggleOscMessage(toggle);

            Assert.AreEqual(OSCCameraEndpoints.TriggerTakesPhotos, message.Address);
