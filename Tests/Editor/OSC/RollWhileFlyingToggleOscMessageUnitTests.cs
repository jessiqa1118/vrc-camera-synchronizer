...
{
    [TestFixture]
    public class RollWhileFlyingToggleOscMessageUnitTests
    {
        [TestCase(true, "T")]
        [TestCase(false, "F")]
        public void Ctor_WithToggle_BuildsOscMessage(bool toggle, string expectedTypeTag)
        {
            var message = new RollWhileFlyingToggleOscMessage(toggle);

            Assert.AreEqual(OSCCameraEndpoints.RollWhileFlying, message.Address);
