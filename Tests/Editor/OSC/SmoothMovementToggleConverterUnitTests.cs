...
namespace VRCCamera.Tests.Unit
{
    ...
    public class SmoothMovementToggleConverterUnitTests
    {
        ...
        {
            Assert.IsTrue(_converter.FromOSCMessage(new Message(OSCCameraEndpoints.SmoothMovement, new[] { new Argument(true) })));
            Assert.IsFalse(_converter.FromOSCMessage(new Message(OSCCameraEndpoints.SmoothMovement, new[] { new Argument(false) })));
            Assert.IsTrue(_converter.FromOSCMessage(new Message(OSCCameraEndpoints.SmoothMovement, new[] { new Argument(1) })));
            Assert.IsFalse(_converter.FromOSCMessage(new Message(OSCCameraEndpoints.SmoothMovement, new[] { new Argument(0) })));
            Assert.IsTrue(_converter.FromOSCMessage(new Message(OSCCameraEndpoints.SmoothMovement, new[] { new Argument(1.0f) })));
            Assert.IsFalse(_converter.FromOSCMessage(new Message(OSCCameraEndpoints.SmoothMovement, new[] { new Argument(0.0f) })));
        }
        ...
        public void FromOSCMessage_InvalidInputs_ReturnsFalse()
        {
            ...
            Assert.IsFalse(_converter.FromOSCMessage(new Message(OSCCameraEndpoints.SmoothMovement, System.Array.Empty<Argument>())));
            Assert.IsFalse(_converter.FromOSCMessage(new Message(OSCCameraEndpoints.SmoothMovement, new[] { new Argument("true") })));
        }
    }
}
