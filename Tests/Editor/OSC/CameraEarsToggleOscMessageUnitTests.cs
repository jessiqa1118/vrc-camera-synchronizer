...
{
    [TestFixture]
    public class CameraEarsToggleOscMessageUnitTests
    {
        [TestCase(true, "T")]
        [TestCase(false, "F")]
        public void Ctor_WithToggle_BuildsOscMessage(bool toggle, string expectedTypeTag)
        {
            var message = new CameraEarsToggleOscMessage(toggle);

            Assert.AreEqual(OSCCameraEndpoints.CameraEars, message.Address);
