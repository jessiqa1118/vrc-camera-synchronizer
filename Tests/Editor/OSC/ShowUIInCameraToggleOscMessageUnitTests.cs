...
{
    [TestFixture]
    public class ShowUIInCameraToggleOscMessageUnitTests
    {
        [TestCase(true, "T")]
        [TestCase(false, "F")]
        public void Ctor_WithToggle_BuildsOscMessage(bool toggle, string expectedTypeTag)
        {
            var message = new ShowUIInCameraToggleOscMessage(toggle);

            Assert.AreEqual(OSCCameraEndpoints.ShowUIInCamera, message.Address);
