...
{
    [TestFixture]
    public class LookAtMeToggleOscMessageUnitTests
    {
        [TestCase(true, "T")]
        [TestCase(false, "F")]
        public void Ctor_WithToggle_BuildsOscMessage(bool toggle, string expectedTypeTag)
        {
            var message = new LookAtMeToggleOscMessage(toggle);

            Assert.AreEqual(OSCCameraEndpoints.LookAtMe, message.Address);
