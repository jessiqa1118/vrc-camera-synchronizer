...
{
    [TestFixture]
    public class DollyPathsStayVisibleToggleOscMessageUnitTests
    {
        [TestCase(true, "T")]
        [TestCase(false, "F")]
        public void Ctor_WithToggle_BuildsOscMessage(bool toggle, string expectedTypeTag)
        {
            var message = new DollyPathsStayVisibleToggleOscMessage(toggle);

            Assert.AreEqual(OSCCameraEndpoints.DollyPathsStayVisible, message.Address);
