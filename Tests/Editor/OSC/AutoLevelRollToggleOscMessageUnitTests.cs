...
{
    [TestFixture]
    public class AutoLevelRollToggleOscMessageUnitTests
    {
        [TestCase(true, "T")]
        [TestCase(false, "F")]
        public void Ctor_WithToggle_BuildsOscMessage(bool toggle, string expectedTypeTag)
        {
            var message = new AutoLevelRollToggleOscMessage(toggle);

            Assert.AreEqual(OSCCameraEndpoints.AutoLevelRoll, message.Address);
