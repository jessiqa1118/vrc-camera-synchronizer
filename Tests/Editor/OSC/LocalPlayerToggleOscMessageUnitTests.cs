...
{
    [TestFixture]
    public class LocalPlayerToggleOscMessageUnitTests
    {
        [TestCase(true, "T")]
        [TestCase(false, "F")]
        public void Ctor_WithToggle_BuildsOscMessage(bool toggle, string expectedTypeTag)
        {
            var message = new LocalPlayerToggleOscMessage(toggle);

            Assert.AreEqual(OSCCameraEndpoints.LocalPlayer, message.Address);
